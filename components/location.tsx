...
export function LocationSection() {
  return (
    <section id="location" className="bg-linen-dark py-20 sm:py-28">
      <div className="mx-auto max-w-6xl px-5">
        ...
        <div className="mt-10 grid grid-cols-1 gap-8 md:grid-cols-2">
          <div className="grid grid-cols-2 gap-3">
            {nearby.map((n) => (
              <div key={n.place} className="rounded-2xl bg-cloud p-5 shadow-card">
                <p className="font-display text-2xl text-brass">{n.time}</p>
                <p className="mt-1 text-xs text-ink/60">{n.place}</p>
              </div>
            ))}
          </div>
          ...
          */}
          <div
            ...
            className="flex h-64 items-center justify-center rounded-2xl border border-dashed border-ink/20 bg-azure-mist/40 md:h-auto"
          >
            <span className="font-mono text-xs uppercase tracking-widest2 text-ink/40">
              Map embed goes here
            </span>
          </div>
        </div>
