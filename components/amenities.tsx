...
export function Amenities() {
  return (
    <section id="experiences" className="bg-ink py-20 text-linen sm:py-28">
      <div className="mx-auto max-w-6xl px-5">
        <p className="eyebrow text-azure-soft">Included</p>
        ...
        <div className="mt-10 grid grid-cols-1 gap-px overflow-hidden rounded-2xl bg-linen/10 sm:grid-cols-2 md:grid-cols-3">
          {experiences.map((e) => (
            <div key={e.title} className="bg-ink p-7">
              <h3 className="font-display text-lg font-medium text-linen">{e.title}</h3>
              <p className="mt-2 text-sm leading-relaxed text-linen/60">{e.detail}</p>
            </div>
          ))}
        </div>
      </div>
