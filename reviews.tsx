...
export function Reviews() {
  return (
    <section id="reviews" className="mx-auto max-w-6xl px-5 py-20 sm:py-28">
      ...
      <p className="mt-3 max-w-md text-sm text-ivory/50">
      ...
      <div className="mt-10 grid grid-cols-1 gap-5 sm:grid-cols-3">
        {sampleReviews.map((r, i) => (
          <figure key={i} className="rounded-2xl border border-dashed border-ivory/20 bg-navy-light/60 p-6">
            <div className="text-gold" aria-hidden="true">★★★★★</div>
            <blockquote className="mt-3 text-sm italic leading-relaxed text-ivory/70">
              &ldquo;{r.quote}&rdquo;
            </blockquote>
            <figcaption className="mt-4 font-mono text-[11px] uppercase tracking-widest2 text-ivory/40">
              {r.name}
            </figcaption>
          </figure>
        ))}
